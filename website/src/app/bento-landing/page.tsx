...
const HeaderNav: React.FC = () => (
  <BentoBox className="flex flex-row justify-between items-center w-full p-0 pl-[3rem]">
    ...
    <nav className="flex items-center gap-[3rem] p-[.5rem] h-full">
      <a href="#" className="hover:text-white text-sm text-gray-400">Pricing</a>
      <a href="#" className="hover:text-white text-sm text-gray-400">Sign In</a>
      <button className="h-full w-[20rem] bg-gradient-to-b from-white to-[#888888] text-black font-semibold rounded-[1rem] transition-all duration-300 hover:from-[#ffffff] hover:to-[#999999] hover:shadow-md">
        <LargeText fontSizeClass="text-[1.4rem]" colorClass="text-black">Download Extension</LargeText>
      </button>
    </nav>
  </BentoBox>
);
...
const CtaButtonBox: React.FC = () => (
  <BentoBox className='p-[.5rem]'>
    <GradientButton className='rounded-[1rem] gap-[1.5rem]'>
      <LargeText fontSizeClass="text-[1.8rem]" colorClass='text-black'>Fix yours now</LargeText>
      <Upload className="w-[var(--button-icon-size)] h-[var(--button-icon-size)]" />
      <SmallText className='text-black'>upload<br />resume</SmallText>
    </GradientButton>
  </BentoBox>
);
...
const PricingBox: React.FC = () => (
  <BentoBox className="items-start text-left p-[1.5rem] pl-[2rem] gap-[1rem]">
    ...
    <div className="flex flex-col items-start gap-[1.5rem] p-[.75rem] pl-[1rem] border-l-2 border-[rgba(121,69,54,0.3)]">
...
const ChecklistItem: React.FC<{
  ...
  <div
    className={twMerge(
      `flex flex-row items-center gap-[var(--main-gap)] w-full h-[4rem] rounded-b-[var(--small-rounding)] bg-gradient-to-b from-transparent from-0% to-neutral-800/20 to-100% pl-[2rem]`,
      className
    )}
  >
